...
if [ -n "$HERDR_PANE_ID" ]; then
  ctx="This session runs inside herdr pane $HERDR_PANE_ID (tab $HERDR_TAB_ID, space $HERDR_WORKSPACE_ID, session ${HERDR_SESSION:-default}). Never close or repurpose this pane or its tab. Once you know what this session is working on, label the tab by running: herdr tab rename $HERDR_TAB_ID '<3-5 word task>'; re-run it if the task materially changes. Herdr is the primary way to run and manage coding agents in the terminal; the herdr-agents skill has the recipes."
