...
When asked for a **summary of what changed** (scope/shape, not a walkthrough), answer with a signature profile instead: code blocks grouped by module in dependency or runtime order, one line per declaration (the signature verbatim, marked `+`/`~`/`-` for added/changed/removed, `// was ...` inline on changes, a trailing comment only where the signature can't carry the meaning). Structs and interfaces compress to their changed members (`~ Workflow { ... ConfigVersion *int64 ... }  // + field`); tests, fixtures, and README churn get one collective closing line. Close with what's untouched and a line or two of prose on what fell out, since consequences don't show in signatures.
