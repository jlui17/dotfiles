...
A herdr session is a new main session: fully steerable by the user, visible in the picker, alive after you're gone. Starting one is user-opt-in; the trigger ("kickoff X") is injected by the herdr SessionStart hook, and delegation inside your own task (searches, review passes, verification, parallel slices) uses the Agent tool and worktrees, not herdr.
