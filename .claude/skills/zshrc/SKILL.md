...
**Startup telemetry**: every interactive shell appends one line to `~/.cache/zsh-startup-log.tsv` (rc/first-prompt/turbo-done ms, load averages, claude process count, terminal context, cold-path flags). When investigating startup latency, read that log first — it answers whether slowness correlates with system load or with the cold paths (daily full compinit, `_cached_eval` regen) before any live measurement.
