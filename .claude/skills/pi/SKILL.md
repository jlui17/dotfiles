...
- Add package: add line to packages.txt (npm:, git:, or local path; see file header), run install.sh
- Remove package: comment/delete line in packages.txt, `pi remove <source>` (install.sh never removes)

**Default voice:** none; the global rules ship to Claude Code only. If pi sessions need Justin's voice, see the voice-sink note in the claude-code module skill.
