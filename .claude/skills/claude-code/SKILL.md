...
`output-styles/*.md` are plain symlinked files (frontmatter and all), not assembled from anything; `justin.md` is selected via `outputStyle` in `claude-code/settings.json`. An output style loads for the main session only, never for subagents, which is why the voice and session-reply rules live there rather than in `rules.d/`: they govern how Claude talks to Justin in the terminal. A rule subagents also need is a fragment, not a style section.
...
- **Progressive disclosure.** Specialized guidance goes in a skill; CLAUDE.md carries the pointer and the trigger, not the content (the output style → style skill split is the in-repo example).
...
- **Consolidate as you grow.** A fragment that outgrows a few paragraphs gets distilled, detail pushed into a skill; additions compete for the generated `~/CLAUDE.md`'s line budget (install.sh owns the number and warns past it). Routing, the addition-is-an-edit rule, and the overlap sweep are canonical in the context-audit skill.
- **Personal facts go to auto-memory, standing rules go to rules.d** (routing canonical in the context-audit skill). Auto-memory covers what one machine's sessions learn; a rule every session must follow still lands as a fragment.
