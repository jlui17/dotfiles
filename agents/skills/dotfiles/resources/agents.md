...
- `agents/rules.d/` holds the shared fragments. They reach both `~/CLAUDE.md` and `${CODEX_HOME:-~/.codex}/AGENTS.md`. A machine with `codex` in `SKIP_MODULES` has no Codex to read that copy, so only `~/CLAUDE.md` is written there.
