# Design critiques / UX walkthroughs

Voice (main doc) holds, with one inversion: **the actor flips from code to *you***. First-person, experiential — what you clicked, what tripped you up, what you'd change. Despite the casual voice, every "Component" grounds in real file paths.

## Fixed section schema (keep order)

- **Component** — file path + what it renders, sub-files parenthetical: "`runs/[run_id]/main.tsx` renders... (`plots.tsx`)".
- **Purpose** — one line: what it's for.
- **What tripped me up** — numbered first-person friction.
- **What I'd change** — bold imperative-verb bullets.
...
## Philosophy before the change list

One line naming what the set optimizes for, pivot italicized:
> My priority here is *simpler and smoother*: clarifying one-liners, fewer steps, small QOL wins.
...
Say what to look at and what's wrong — never "here's a screenshot."
> Top summary, with the metric list on the left, a large empty space on the right, and the real data hidden behind `SHOW DETAILS`:
