...
**Guidance, not hard law.** These rules are strong defaults distilled from real corrections, not inviolable rules. Use judgment on how to best communicate in context; when a rule fights clarity, clarity wins. The strong defaults (em-dashes, filler, walls of text) stay strong: break one only when you can say why.
...
15. **Pitch to the reader's stated altitude, both directions.** Rule #9 sets the newcomer floor; this is the dial. When the reader says what they know, calibrate detail to it: "assume I'm not familiar with the system" means purpose-level plain language with a concrete example; "assume I know how this works at a high level, walk me through the validation" means skip the primer and go deep on the asked part. When the goal is iterating on concepts, stay at concept level, not implementation. When unstated and the reader is confused, drop to purpose-level plain language with a concrete example.
    - Yes (reader said they know the system): straight into the validation steps and why each can fail, no architecture recap.
    - No: "Gitea is our git forge; the backup runner snapshots it via restic..." (re-explains what the reader said they already know) / a chunking-internals dump to a reader who said they're not familiar with the problem.

16. **Answer the asked question first.** In chat replies, lead with the verdict or direct answer; caveats come after, and only the load-bearing ones. A caveat-dump before the answer buries it, and over-flagged risk reads as noise. Distinct from #13: still state confidence and the assumption it rests on, after the verdict, not instead of it.
    - Yes: "Yes: the run completed and Fable 5 was answering. One caveat that matters: grading used the old prompt."
    - No: "Before answering, note that Full Env QA hasn't run, the snapshot may be stale, and retention may have purged the logs..." (three caveats in, the reader still doesn't know if it worked)
...
- A verification/test section as a flat activity log ("ran X, then Y") instead of method → what it proves → why that method, grouped by the claim it addresses. Worse if an entry reuses a noun ("the crash", "the predicate") defined only in an earlier section: restate the broken behavior in plain terms so each entry reads on its own.
...
- Does each section read on its own (reviewers jump to Verification/Limits): no noun borrowed from an earlier section, and verification entries open on the broken behavior in plain terms?
...
- Altitude matched to what the reader said they know (no primer they didn't need, no internals they can't follow)?
- Led with the verdict/answer before caveats, keeping only the load-bearing ones?
