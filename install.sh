...
# -- Output plumbing --------------------------------------------------------
# open_log, the output API and run_module, shared with update_pkgs.
OUTPUT_LOG="/tmp/dotfiles-install.log"
...
# repo reads (agents/external-skills.txt, plugins.txt) shares this shape;
...
# <slug>, from both fragment dirs. The NN- prefix is ordering only; slugs are
# what SKIP_RULES names.
rule_section_slugs() {
  local -a names=("$DOTFILES_DIR/agents/rules.d/"*.md(N:t) "$DOTFILES_DIR/claude-code/rules.d/"*.md(N:t))
  local n
  for n in "${(o)names[@]}"; do
    echo "${${n%.md}#*-}"
  done
...
# ~/.codex/AGENTS.md. Slugs come from agents/rules.d/ and claude-code/rules.d/
# filenames. Available:
...
# provides bunx for the external-skills replay in setup_agents.
#
...
# CLI path live in a drop-in. Ubuntu waits for its Tailscale bind address.
write_t3_service_dropin() {
  local dropin_dir="$1/t3code.service.d"
  local bind_host=127.0.0.1 tmp
  if [[ "$OS" == "ubuntu" ]]; then
...
EOF
  ...
  cat >> "$tmp" <<EOF
[Service]
Environment=T3CODE_HOST=$bind_host
...
# Build a global rules file from the *.md fragments of the given dirs, in
# filename order across dirs, dropping fragments whose slug is in the machine's
# SKIP_RULES (per-machine, .dotfiles-local). Generated instead of symlinked
# because per-machine section exclusion needs a per-machine artifact — a
# symlink is all-or-nothing. 99-local.md (gitignored) rides along for
# machine-only rules. Quiet when the output is already current.
assemble_global_rules() {
  local dst="$1"
  shift
  local dir fragment name slug tmp
  local -a source_dirs=("${@#$DOTFILES_DIR/}")
  local -A fragment_by_name
  for dir in "$@"; do
    for fragment in "$dir/"*.md(N); do
      name="${fragment:t}"
      if [[ -n "${fragment_by_name[$name]}" ]]; then
        warn "$name exists in both ${${fragment_by_name[$name]:h}#$DOTFILES_DIR/} and ${dir#$DOTFILES_DIR/}; using the former."
        continue
      fi
      fragment_by_name[$name]="$fragment"
    done
  done
  tmp="$(mktemp)"
  {
    echo "<!-- Generated by dotfiles install.sh from ${(j:/, :)source_dirs}/; edit the"
    echo "     fragments there and re-run install.sh; direct edits here get overwritten. -->"
    for name in "${(ok)fragment_by_name[@]}"; do
      slug="${${name%.md}#*-}"
      (( ${SKIP_RULES[(Ie)$slug]} )) && continue
      echo ""
      cat "${fragment_by_name[$name]}"
    done
  } > "$tmp"
  local lines
  lines=$(wc -l < "$tmp")
...
install_generated_file() {
  ...
  if [[ -L "$dst" ]]; then
    ...
    note "Moved the hand-written $(basename "$dst") aside as $dst.bak; merge what you still want into a fragment."
  fi
  mv "$tmp" "$dst"
...
# Only what Claude Code alone reads. Global skills, external skills, and the
# generated rules files are shared with every harness, so they live in the
# agents module (setup_agents); other harnesses get their own modules
# (setup_codex, setup_opencode).
setup_claude_code() {
...
# The context every harness shares: global skills, external skills, and the
# generated rules files. Skills are linked once per harness directory: Codex
# reads ~/.agents/skills natively; Claude Code discovers skills in
# ~/.claude/skills only, so each skill is linked there too.
setup_agents() {
  echo "==> Shared agent context (global skills, external skills, rules)..."
  local skills_dir="$HOME/.agents/skills"
  local claude_skills_dir="$HOME/.claude/skills"
  local -a desired=()
  local skipped name

  # Machines hold their 99-local.md at the claude-code path from before this
  # module assembled the rules.
  local old_local="$DOTFILES_DIR/claude-code/rules.d/99-local.md"
  local new_local="$DOTFILES_DIR/agents/rules.d/99-local.md"
  if [[ -f "$old_local" && ! -e "$new_local" ]]; then
    mv "$old_local" "$new_local"
    changed "moved 99-local.md to agents/rules.d"
  elif [[ -f "$old_local" ]]; then
    warn "claude-code/rules.d/99-local.md is superseded by agents/rules.d/99-local.md; merge it there and delete it."
  fi
  # Retired layouts left ~/AGENTS.md behind, first as a link into the repo,
  # later as a generated file; no harness reads that path. A hand-written one
  # is not ours to remove.
  if [[ -L "$HOME/AGENTS.md" && "$(readlink "$HOME/AGENTS.md")" == "$DOTFILES_DIR"/* ]] ||
     { [[ -f "$HOME/AGENTS.md" && ! -L "$HOME/AGENTS.md" ]] && grep -q "Generated by dotfiles install.sh" "$HOME/AGENTS.md" 2>/dev/null; }; then
    rm "$HOME/AGENTS.md"
    changed "removed retired ~/AGENTS.md"
  fi
  ...
  # External skills (agents/external-skills.txt) come from other people's
  ...
  local manifest="$DOTFILES_DIR/agents/external-skills.txt"
  if ! command_exists bunx; then
    warn "bunx not found — skipping external skills."
    note "External skills are declared in agents/external-skills.txt; add bun = \"latest\" to ~/.config/mise/config.toml, run mise install, and re-run."
  else
    ...
    while IFS= read -r line; do
      ...
      for skill in ${=line#$source}; do
        ...
        ext_skills+=("$skill")
        [[ -e "$claude_skills_dir/$skill" ]] || new_skills+=("$skill")
      done
      (( ${#ext_skills[@]} )) || continue
