...
#   SKIP_RULES    — global-rules sections (claude-code-skills/rules.d/ slugs) left
#                   out of this machine's generated ~/CLAUDE.md
#   SKIP_SKILLS   — global skills (claude-code-skills/skills/ dirs) not linked into
...
MODULES=(
  ...
  pi:setup_pi
  claude-code-skills:setup_claude_code_skills
...
rule_section_slugs() {
  local f
  for f in "$DOTFILES_DIR/claude-code-skills/rules.d/"*.md(N); do
    echo "${${${f:t}%.md}#*-}"
  done
}

# Names of the global skills, one per line: claude-code-skills/skills/<name>/ →
# <name>. These are what SKIP_SKILLS names.
global_skill_names() {
  local d
  for d in "$DOTFILES_DIR/claude-code-skills/skills/"*/(N); do
    echo "${${d%/}:t}"
  done
}
...
OUTPUT_STYLE_RULES=(style explaining-changes)
...
# Slugs come from claude-code-skills/rules.d/ filenames. Available:
...
# claude-code-skills/skills/ directories. Available:
...
assemble_global_rules() {
  ...
  tmp="$(mktemp)"
  {
    echo "<!-- Generated by dotfiles install.sh from claude-code-skills/rules.d/ — edit the"
    echo "     fragments there and re-run install.sh; direct edits here get overwritten. -->"
    for fragment in "$DOTFILES_DIR/claude-code-skills/rules.d/"*.md(N); do
      slug="${${${fragment:t}%.md}#*-}"
...
---
<!-- Generated by dotfiles install.sh from claude-code-skills/rules.d/ — edit the
...
setup_claude_code_skills() {
  ...
  local module_dir="$DOTFILES_DIR/claude-code-skills"
  local skipped
