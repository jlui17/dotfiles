...
rule_section_slugs() {
  local f
  for f in "$DOTFILES_DIR/agent-skills/rules.d/"*.md(N); do
    echo "${${${f:t}%.md}#*-}"
  done
}
...
# minus SKIP_RULES. Generated instead of symlinked because per-machine section
...
assemble_global_rules() {
  local dst="$1" fragment slug tmp
  tmp="$(mktemp)"
  {
    echo "<!-- Generated by dotfiles install.sh from agent-skills/rules.d/ — edit the"
...
setup_agent_skills() {
  ...
  # $HOME (identical content, minus this machine's SKIP_RULES sections).
  assemble_global_rules "$HOME/CLAUDE.md"
  assemble_global_rules "$HOME/AGENTS.md"
  echo ""
}
